...
c, round(avg(rows)) from system.part_log where event_date >= today() and event_type = 'NewPart' --and event_time > now() - 3600 group by database, table, t order by t ``` # 6\.6 \- Remove block numbers from zookeeper for removed partitions
