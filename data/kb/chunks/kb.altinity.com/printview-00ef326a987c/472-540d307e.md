...
``` [mysqld] information_schema_stats_expiry=0 ``` Or setup it via SQL query: ``` SET GLOBAL information_schema_stats_expiry=0; ``` # 11\.6 \- Partial updates Partial updatesClickHouse® is able to fetch from a source only updated rows. You need to define `update_field` section.
...
# 11\.7 \- range\_hashed example \- open intervals
