...
— stabilizes memory usage and prevents excessive RAM consumption during merges in `system.metric_log` and similar tables. # 5\.22 \- Precreate parts using clickhouse\-local Precreate parts using clickhouse\-local.## Precreate parts using clickhouse\-local the code below were testes on 23\.3
...
```
# 5\.23 \- Recovery after complete data loss
