...
stop that quickly if needed) clickhouse-local --path=. --query "SELECT count() FROM numbers(100) WHERE sleepEachRow(0.1) SETTINGS max_block_size=1" ls -la data/local/test/ clickhouse-local --path=. --query "SELECT _part,* FROM local.test ORDER BY id" ``` # 5\.23 \- Recovery after complete data loss
