...
# 1\.3 \- Adjusting librdkafka settings Adjusting librdkafka settings- To set rdkafka options \- add to `<kafka>` section in `config.xml` or preferably use a separate file in `config.d/`: - <https://github.com/edenhill/librdkafka/blob/master/CONFIGURATION.md> Some random example using SSL certificates to authenticate:
