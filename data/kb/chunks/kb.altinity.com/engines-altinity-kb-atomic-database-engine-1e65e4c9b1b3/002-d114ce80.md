...
Ordinary for new setups? All things inside ClickHouse itself should work smoothly with `Atomic`. But some external tools \- backup tools, things involving other kinds of direct manipulations with ClickHouse files \& folders may have issues with `Atomic`.
