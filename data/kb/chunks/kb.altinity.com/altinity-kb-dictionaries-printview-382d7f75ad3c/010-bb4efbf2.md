...
│ first row │ └────────────────────────────────────┘ ``` # 9 \- SPARSE\_HASHED VS HASHED vs HASHED\_ARRAY SPARSE\_HASHED VS HASHED VS HASHED\_ARRAYSparse\_hashed and hashed\_array layouts are supposed to save memory but has some downsides. We can test it with the following:
