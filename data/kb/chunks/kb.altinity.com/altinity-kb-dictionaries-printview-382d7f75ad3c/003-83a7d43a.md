...
LIFETIME(MIN 1 MAX 1000) LAYOUT(FLAT()); SELECT dictGet(concat(currentDatabase(), '.flat_dictionary'), 'value_en', number + 1), dictGet(concat(currentDatabase(), '.flat_dictionary'), 'value_ru', number + 1) FROM numbers(3); ``` # 3 \- Dimension table design Dimension table design## Dimension table design considerations ### Choosing storage Engine
