...
\- Operations and Troubleshooting Runtime tuning, resource settings, and error diagnostics.# 9\.4\.1 \- Setting the background message broker schedule pool size Guide to managing the `background_message_broker_schedule_pool_size` setting for Kafka, RabbitMQ, and NATS table engines in your database.## Overview
