...
DateTime, .... INDEX ix_B (B) type minmax GRANULARITY 3 ) engine = MergeTree partition by toYYYYMM(ts) order by (toStartOfDay(ts),A,B); ``` bloom\_filter index type instead of min\_max could work fine in some situations. ### Inverted index as a projection
