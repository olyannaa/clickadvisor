...
VIEW my_new_view (c1 Int, c2 String, ...) AS SELECT * FROM deals WHERE category_id IN ( SELECT category_id FROM deal_categories WHERE category = getSetting('my2_category') ); ``` # 32 \- Use both projection and raw data in single query
