...
toIntervalMonth(6) RECOMPRESS CODEC(ZSTD(6)); ``` All columns have implicit default compression from server config, except `event_time`, that’s why need to change to compression to `Default` for this column otherwise it won’t be recompressed. # 45 \- UPDATE via Dictionary
