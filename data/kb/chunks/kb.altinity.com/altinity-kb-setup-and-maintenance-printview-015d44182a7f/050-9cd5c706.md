...
which order those sources are checked (precedence). In short: it is the access\-storage routing configuration for users/roles/policies/profiles/quotas. Apply on **every** ClickHouse node: ``` <clickhouse> <user_directories replace="replace"> <users_xml> <path>/etc/clickhouse-server/users.xml</path> </users_xml> <replicated> <zookeeper_path>/clickhouse/access/</zookeeper_path> </replicated> </user_directories> </clickhouse> ``` Why `replace="replace"` matters:
