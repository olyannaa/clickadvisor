...
query plus enable queuing of queries (`queue_max_wait_ms`) if the `max_concurrent_queries` limit is exceeded. Also `max_block_size` is not usually touched but in this case we can lower it ro reduce RAM usage. # 43 \- Converting MergeTree to Replicated
