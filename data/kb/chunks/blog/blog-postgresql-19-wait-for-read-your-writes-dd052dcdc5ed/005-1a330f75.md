...
## Conclusion \# What I like about `WAIT FOR` is that it keeps asynchronous replication asynchronous. Instead of making every write wait for a replica, the cost is paid only when a read actually needs the read\-your\-writes guarantee.
...
[Sign up](https://console.clickhouse.cloud/signUp?intent=pg&loc=blog-cta-1640-get-started-with-clickhouse-managed-postgres-today-sign-up&utm_blogctaid=1640)### Get started today
