...
the primary data structure that can be updated dynamically with new data. ``` 1import perspective from "https://cdn.jsdelivr.net/npm/@finos/perspective@3.0.0/dist/cdn/perspective.js"; 2const forex_worker = await perspective.worker(); 3// fetch some rows... 4const forex_table = await market_worker.table(rows, { limit: 20000 }) ``` Copy command
...
```
1const forex_table = await market_worker.table(rows, { limit: 20000 });
```
Copy command
