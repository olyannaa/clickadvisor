...
```
1SELECT DISTINCT Tail_Number FROM ontime WHERE Tail_Number != '' ORDER BY Tail_Number
2-- Top results: #N/A, '144DA, (null), -N037M
3
4SELECT DISTINCT Tail_Number FROM ontime WHERE Tail_Number != '' ORDER BY Tail_Number DESC
5-- Top results: �NKNO�, XXXXXX, Unknow, UNKNOW
```
Copy command
...
```
1SELECT DISTINCT Tail_Number FROM ontime WHERE match(Tail_Number,
2'^[A-Z]+[0-9]+[A-Z0-9]*$') ORDER BY Tail_Number
3-- Top results: A367NW, D942DN, M67153, etc. - all valid
```
Copy command
