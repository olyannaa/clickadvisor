...
IP Ranges to CIDR Notation \# Because our `ip_trie` dictionary requires IP address ranges to be expressed in CIDR notation, we'll need to transform `ip_range_start` and `ip_range_end`. For reference, here's a brief [description of CIDR notation from Wikipedia](https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing):
...
![cidr_range_calculation.png](/_next/image?url=%2Fuploads%2Fcidr_range_calculation_398d9ddf49.png&w=2048&q=75)
