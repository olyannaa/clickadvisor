...
**applied independently for each data range** in each data stream, ensuring that updates are applied correctly without disrupting parallelism. *(If needed, you can fully materialize all accumulated not\-yet\-merged changes with `ALTER TABLE … APPLY PATCHES`, but that’s optional.)*
