...
by_date; ``` Copy command We’ll now run our previous query that finds properties sold in Manchester in January 2023 against each table. We’ll run it against each table three times, recording the lowest time and the rows processed:
...
```
1┌─granulesWithMatchingRows─┬─totalGranules─┬──pct─┐
2│                    29724 │         29755 │ 99.9 │
3└──────────────────────────┴───────────────┴──────┘
```
Copy command
