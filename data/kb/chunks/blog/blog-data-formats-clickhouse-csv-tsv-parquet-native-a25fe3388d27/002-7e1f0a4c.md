...
command ### Importing data from broken or custom CSV files \# In cases where the CSV file is encoded in a non\-standard way or just invalid, we can use the [CustomSeparated](https://clickhouse.com/docs/en/sql-reference/formats/#format-customseparated) format to customize escaping rules and delimiters:

```
1SET format_custom_field_delimiter = '|';
2SET format_custom_row_between_delimiter = ';';
3SET format_custom_escaping_rule = 'JSON';
```
Copy command
...
```
1INSERT INTO some_table FROM INFILE 'custom.csv' FORMAT CustomSeparated
```
Copy command
...
```
1clickhouse-client -q "INSERT INTO some_table FORMAT TabSeparated" < 
2data.tsv
```
Copy command
...
```
1clickhouse-client - q "INSERT INTO sometable FORMAT JSONEachRow" < 
2access.log
```
Copy command
...
```
1INSERT INTO some_log FROM INFILE 'custom.txt'
2SETTINGS
3  format_regexp = '([0-9]+?) \[(.+?)\] \- "(.+)"'
4FORMAT Regexp
```
