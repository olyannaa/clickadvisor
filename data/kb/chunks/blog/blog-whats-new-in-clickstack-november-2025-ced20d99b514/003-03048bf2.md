...
![image3.png](/_next/image?url=%2Fuploads%2Fimage3_3985f9dc68.png&w=2048&q=75)
...
```
1ORDER BY (toStartOfMinute(Timestamp), StatusCode, flow, country, ServiceName, Timestamp)
```
Copy command
...
```
1ORDER BY (toStartOfMinute(Timestamp), empty(ParentSpanId), StatusCode, flow, country, ServiceName, Timestamp)
```
Copy command
