...
it sees that the global state version bumped (e.g., from 5 to 6\), it: - Fetches the new state from Keeper. - Merges changes into local memory. - Updates its `/replicas` version in Keeper to signal it’s up\-to\-date.
