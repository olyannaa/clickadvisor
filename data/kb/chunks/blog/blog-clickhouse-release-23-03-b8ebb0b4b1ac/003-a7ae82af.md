...
```
1WHERE parallel_replicas_custom_key % NODE_COUNT = NODE_NUM
```
Copy command
...
```
1WHERE parallel_replicas_custom_key >= NODE_NUM * (MAX_OF_VALUE_SPACE / NODE_COUNT)
2  AND parallel_replicas_custom_key < (NODE_NUM + 1) * (MAX_OF_VALUE_SPACE / NODE_COUNT)
```
Copy command
