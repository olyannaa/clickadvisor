...
Insert the data 22INSERT INTO trips_small_pk SELECT * FROM trips_small_inferred ``` Copy command We then rerun our queries. We compile the results from the three experiments to see the improvements in elapsed time, rows processed, and memory consumption.
