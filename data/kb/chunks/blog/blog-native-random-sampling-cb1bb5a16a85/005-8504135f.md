...
bucket it lands in will be heavily skewed. By contrast, there are 1\.32 million distinct postcode1/postcode2 combinations, which is enough to let the hash function do its job of distributing the data evenly. ## Creating the table \#
...
```
1┌─count()─┐
2│ 3106578 │ -- 3.11 million
3└─────────┘
4
51 row in set. Elapsed: 0.043 sec.
```
Copy command
