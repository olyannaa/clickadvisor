...
rows, 12.00 GB (3.79 billion rows/s., 45.53 GB/s.) [✎](https://sql.clickhouse.com?query_id=7FM7GEM6MGK3I4Y916BRKK) ``` Note how we’ve used a window max() to compute the max hits per hour, passing this to the `bar()` function for visualization. ## Counters and Gauge metrics \#
