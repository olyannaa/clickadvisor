...
total_downloads FROM rubygems.downloads WHERE (gem = 'bundler') AND timestamp > now() - INTERVAL 2 DAYS GROUP BY hour ORDER BY hour ASC ``` Copy commandRun query ### Downloads by Ruby version over time \# For the bundler gem:
