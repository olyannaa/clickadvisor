...
are more than 90 functions, so we won't list them all here. ## The type system \# As of the 26\.1 release, ClickHouse has six [concrete geometry types](https://clickhouse.com/docs/en/sql-reference/data-types/geo). Each one is a named alias over a primitive type:
...
2──────
3tuple:                  (51.5,-0.12)
4toTypeName(tuple):      Tuple(Float64, Float64)
5point:                  (51.5,-0.12)
6toTypeName(point):      Point
7arr:                    [(0,0),(1,1),(2,0)]
8toTypeName(arr):        Array(Tuple(UInt8, UInt8))
9linestring:             [(0,0),(1,1),(2,0)]
10toTypeName(linestring): LineString
```
Copy command
