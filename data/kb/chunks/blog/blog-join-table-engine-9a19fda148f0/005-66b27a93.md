...
`MergeTree` tables, making them suitable for frequent updates. - In particular, for dimensional modeling in ClickHouse Cloud, use `Join(ANY, LEFT, id)` — upserting, deduplication, and data compaction is automatically handled by the underlying `ReplacingMergeTree`! ### Get started today
