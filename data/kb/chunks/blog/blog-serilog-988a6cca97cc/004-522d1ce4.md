...
idx_message message TYPE text(tokenizer = splitByNonAlpha, preprocessor = lowerUTF8(message)) GRANULARITY 8") 26 .WithEngine(@"ENGINE = MergeTree 27 ORDER BY (level, timestamp) 28 TTL timestamp + toIntervalDay(30)"), 29 flushInterval: TimeSpan.FromSeconds(2)); 30}); ``` Copy command Let's break down the schema builder:
