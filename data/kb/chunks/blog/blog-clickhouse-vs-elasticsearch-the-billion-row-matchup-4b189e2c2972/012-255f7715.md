...
and calculate the aggregation result from scratch. ### Query Runtimes \# We run all [benchmark queries](/blog/clickhouse_vs_elasticsearch_the_billion_row_matchup#benchmark-queries) over the data sets that: - Are compressed with the Elasticsearch and ClickHouse standard `LZ4` codec - Don’t store `_source` in Elasticsearch
