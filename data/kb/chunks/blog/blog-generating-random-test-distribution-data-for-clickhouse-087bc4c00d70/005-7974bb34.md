...
1 │ 75179 │ 4└───┴───────┘ ``` Copy command | ``` 1SELECT 2 floor(randBernoulli(0.75)) AS k, 3 count(*) as c 4FROM numbers(100000) 5GROUP BY k 6ORDER BY k ASC ``` Copy commandRun query | ### Generating random data \#
...
```
1CREATE TABLE purchases
2(
3    `dt` DateTime,
4    `customer_id` UInt32,
5    `total_spent` Float32
6)
7ENGINE = MergeTree
8ORDER BY dt
```
...
```
1INSERT INTO purchases SELECT
2    now() - randUniform(1, 1000000.),
3    number,
4    15 + round(randExponential(1 / 10), 2)
5FROM numbers(1000000)
```
Copy command
...
| pruchases_distribution.png | ``` 1SELECT 2    floor(total_spent) AS s, 3    count(*) AS n, 4    bar(n, 0, 350000, 50) 5FROM random.purchases 6GROUP BY s 7ORDER BY s ASC ``` Copy commandRun query |
| --- | --- |
