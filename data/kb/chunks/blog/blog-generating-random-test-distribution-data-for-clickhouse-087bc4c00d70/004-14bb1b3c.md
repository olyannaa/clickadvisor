...
| [`randStudentT()`](https://clickhouse.com/docs/en/sql-reference/functions/random-functions/#randstudentt)  Similar to normal distribution, but with a longer “tail”. | randStudentT.png | ``` 1SELECT 2    floor(randStudentT(4.5)) AS k, 3    bar(count(*), 0, 10000, 10) AS b1 4FROM numbers(100000) 5GROUP BY k 6ORDER BY k ASC ``` Copy commandRun query |
...
| [`randBernoulli()`](https://clickhouse.com/docs/en/sql-reference/functions/random-functions/#randbernoulli)  Binary distribution, useful for modeling failures and successes of a specific operation. | ``` 1┌─k─┬─────c─┐ 2│ 0 │ 24821 │ 3│ 1 │ 75179 │ 4└───┴───────┘ ``` Copy command | ``` 1SELECT 2    floor(randBernoulli(0.75)) AS k, 3    count(*) as c 4FROM numbers(100000) 5GROUP BY k 6ORDER BY k ASC ``` Copy commandRun query |
