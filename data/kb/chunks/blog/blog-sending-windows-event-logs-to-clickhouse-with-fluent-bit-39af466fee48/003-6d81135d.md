...
downloaded [here](https://download.sysinternals.com/files/Sysmon.zip). Just extract the ZIP file in a directory, such as ‘*C:\\Tools\\Sysmon*’. After extraction, you can start the installation with default configuration settings. ``` 1sysmon.exe -accepteula -i ``` Copy command ### Communities bring Open Source together \#
...
```
1sysmon.exe -c sysmonconfig-export.xml
```
Copy command
...
```
1Expand-Archive .\fluent-bit-2.0.6-win64.zip C:\Tools
```
Copy command
