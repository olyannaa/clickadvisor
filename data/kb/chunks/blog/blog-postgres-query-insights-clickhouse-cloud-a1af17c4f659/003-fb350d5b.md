...
or **P99** or **Avg Duration** descending. The top row: ``` 1SELECT * 2FROM orders 3JOIN customers ON orders.customer_id = customers.id 4WHERE orders.status = $1 5ORDER BY orders.created_at DESC 6LIMIT $2; ``` Copy command **4\. Open the pattern flyout.**
