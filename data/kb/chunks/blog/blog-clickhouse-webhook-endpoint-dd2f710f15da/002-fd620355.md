...
2 received_at DateTime DEFAULT now(), 3 raw_payload String, 4 signature String DEFAULT getClientHTTPHeader('X-Hub-Signature-256') 5) ENGINE = MergeTree() 6ORDER BY received_at 7SETTINGS allow_get_client_http_header=1; ``` Copy command The `allow_get_client_http_header` setting is required because reading client headers is disabled by default.
...
```
1http://localhost:8123/?user=webhook_receiver&query=INSERT+INTO+webhook_staging+(raw_payload)+FORMAT+RawBLOB
```
Copy command
The key parts:
