...
of the `sum` function execution with constants 1 and 2\. If we compile this example with gcc and explore binary using [objdump](https://en.wikipedia.org/wiki/Objdump), we can extract `sum` function assembly. ``` 1g++ --version 2g++ (Ubuntu 10.3.0-1ubuntu1~20.04) 10.3.0 ``` Copy command
