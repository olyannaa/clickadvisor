...
BY ( 8 data.kind, 9 fromUnixTimestamp64Micro(data.time_us)) 10SETTINGS index_granularity_bytes = 0; ``` Copy command Note: Setting `index_granularity_bytes = 0` disables the [adaptive granularity](https://clickhouse.com/docs/whats-new/changelog/2019#experimental-features-1) threshold. Not recommended in production, used here only to get a fixed granule size for clarity.
