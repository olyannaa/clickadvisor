...
'QueryFinish' 7 AND query_id = '5ccec420-6f13-43c2-959e-403054d9243a'; ``` ``` ``` ``` ┌─num_parts_with_cache_hits─┬─num_parts_with_cache_misses─┐ │ 5 │ 0 │ └───────────────────────────┴─────────────────────────────┘ ``` ``` For all 5 data parts of the queried table ClickHouse found entries in the cache. ## Wrapping up [\#](/blog/introducing-the-clickhouse-query-condition-cache#wrapping-up)
