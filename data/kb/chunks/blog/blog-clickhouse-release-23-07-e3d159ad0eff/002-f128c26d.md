...
```
1CREATE TABLE uk_house_price
2ENGINE = MergeTree
3ORDER BY (postcode1, postcode2, addr1, addr2)
4SETTINGS allow_nullable_key = 1 AS
5SELECT *
6FROM s3('https://datasets-documentation.s3.eu-west-3.amazonaws.com/uk-house-prices/parquet/house_prices.parquet')
7
80 rows in set. Elapsed: 40.550 sec. Processed 28.28 million rows, 4.67 GB (697.33 thousand rows/s., 115.15 MB/s.)
```
Copy command
...
```
1SELECT *
2FROM uk_house_price
3INTO OUTFILE 'london-prices.parquet'
4
528276228 rows in set. Elapsed: 19.901 sec. Processed 28.20 million rows, 4.66 GB (1.42 million rows/s., 233.98 MB/s.)
```
Copy command
...
2FROM uk_house_price
3INTO OUTFILE 'london-prices.parquet'
428276228 rows in set. Elapsed: 11.649 sec. Processed 28.24 million rows, 4.66 GB (2.42 million rows/s., 400.39 MB/s.)
```
Copy command
