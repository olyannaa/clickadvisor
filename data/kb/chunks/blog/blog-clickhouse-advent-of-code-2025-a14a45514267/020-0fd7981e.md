...
rectangle fit *inside* the boundary polygon without edge alignment errors. ``` 1-- Create slightly inset test bounds (0.01 units inside) 2(least(x1, x2) + 0.01, least(y1, y2) + 0.01) AS bottom_left, ... 3polygonsWithinCartesian(test_bounds, all_points_ring) ``` Copy command **Full Solution:**
...
```
Copy commandRun query[View full puzzle description](https://adventofcode.com/2025/day/9)
