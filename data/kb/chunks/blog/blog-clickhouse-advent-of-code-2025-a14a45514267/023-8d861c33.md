...
FROM part2_minimum_solutions 246) 247 248-- Combine results from both parts 249SELECT * FROM combined_solutions settings use_query_cache=true, query_cache_share_between_users = 1, query_cache_nondeterministic_function_handling = 'save', query_cache_ttl = 80000000, result_overflow_mode = 'throw', read_overflow_mode = 'throw'; ``` Copy commandRun query[View full puzzle description](https://adventofcode.com/2025/day/10)
...
```
1paths.visited_dac OR (edges.to_node = kn.dac_node) AS visited_dac
```
Copy command
**Full Solution:**
