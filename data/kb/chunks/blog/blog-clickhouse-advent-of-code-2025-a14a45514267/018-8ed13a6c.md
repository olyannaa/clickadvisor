...
on every row, we used `uniqCombinedState` to create a compact sketch of unique elements, and `runningAccumulate` to merge these sketches row\-by\-row, providing a running count of unique points efficiently. ``` 1runningAccumulate(points_state) AS unique_points_seen ``` Copy command **Full Solution:**
...
```
Copy commandRun query[View full puzzle description](https://adventofcode.com/2025/day/8)
