...
a range of integers (0 to 2^N) and used `ARRAY JOIN` to explode them into rows. This created a row for every possible permutation of button presses. ``` 1ARRAY JOIN range(0, toUInt32(pow(2, num_buttons))) ``` Copy command **Full Solution:**
...
```
Copy commandRun query[View full puzzle description](https://adventofcode.com/2025/day/10)
