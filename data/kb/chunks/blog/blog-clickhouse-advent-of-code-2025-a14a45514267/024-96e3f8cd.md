...
recursive table to track state. This allowed us to solve the "must visit X and Y" constraint in a single pass without needing complex post\-processing. ``` 1paths.visited_dac OR (edges.to_node = kn.dac_node) AS visited_dac ``` Copy command **Full Solution:**
...
```
Copy commandRun query[View full puzzle description](https://adventofcode.com/2025/day/11)
