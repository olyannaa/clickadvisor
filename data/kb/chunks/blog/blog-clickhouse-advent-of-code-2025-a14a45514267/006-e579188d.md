...
length. If the lambda returns 1 for any length, the ID is flagged. ``` 1arrayExists( 2 x -> (string_length % x = 0) AND (repeat(substring(..., x), ...) = number_string), 3 range(1, string_length) 4) ``` Copy command **Full Solution:**
...
```
Copy commandRun query[View full puzzle description](https://adventofcode.com/2025/day/2)
