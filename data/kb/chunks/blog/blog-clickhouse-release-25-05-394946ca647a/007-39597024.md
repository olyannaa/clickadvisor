...
AS lookup, 2 (SELECT wiki_id FROM wikiEmbeddings WHERE id = 120356) AS lookup_wiki_id 3SELECT id, title, text, url, L2Distance(emb, lookup) AS dist 4FROM wikiEmbeddings 5WHERE wiki_id <> lookup_wiki_id 6ORDER BY dist 7LIMIT 3 8FORMAT Vertical; ``` Copy command
