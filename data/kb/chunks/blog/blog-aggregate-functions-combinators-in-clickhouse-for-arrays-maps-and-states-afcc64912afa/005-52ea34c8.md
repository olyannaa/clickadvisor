...
``` We've used the `sumIfOrNull()` combined function to calculate only declined payments and return `NULL` on an empty set. The `countIfDistinctOrNull()` function counts distinct `total_amount` values but only for rows meeting the specified condition. ## Aggregating arrays \#
...
![Array combinator](/_next/image?url=%2Fuploads%2F6_1320d1a6ee.png&w=2048&q=75)
