...
representation without physically materializing the repeated string column. ``` SELECT sum(cityHash64(t1.o_comment)) FROM orders AS t1 INNER JOIN orders AS t2 ON t1.o_custkey = t2.o_custkey SETTINGS enable_lazy_columns_replication = 1, allow_special_serialization_kinds_in_output_formats = 1; ``` Copy command Fastest of three runs:
...
![Blog-JOINS-improvements.006.png](/_next/image?url=%2Fuploads%2FBlog_JOINS_improvements_006_d7c0b8e7ae.png&w=2048&q=75)
