...
Most importantly, many threads per query will reduce the number of queries per second that can be run. For example, using the [UK Property Price Paid dataset](https://clickhouse.com/docs/en/getting-started/example-datasets/uk-price-paid), we can check what the most popular street names are using:

```
1SELECT count(), street FROM uk_price_paid
2GROUP BY street ORDER BY count() DESC
```
Copy command
...
```
1clickhouse-benchmark -c 10 -i 100 -q "<query>”
```
Copy command
