...
and full text. We first run it with lazy materialization disabled (and cold filesystem cache): ``` 1SELECT helpful_votes, product_title, review_headline, review_body 2FROM amazon.amazon_reviews 3ORDER BY helpful_votes DESC 4LIMIT 3 5FORMAT Null 6SETTINGS query_plan_optimize_lazy_materialization = false; ``` Copy command

```
10 rows in set. Elapsed: 219.071 sec. Processed 150.96 million rows, 71.38 GB (689.08 thousand rows/s., 325.81 MB/s.)
2Peak memory usage: 1.11 GiB.
```
Copy command
...
```
10 rows in set. Elapsed: 0.139 sec. Processed 150.96 million rows, 1.81 GB (1.09 billion rows/s., 13.06 GB/s.)
2Peak memory usage: 3.80 MiB.
```
Copy command
