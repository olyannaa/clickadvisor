...
detected agents automatically 5clickhousectl skills --detected-only ``` Copy command ## Get started \# `clickhousectl` is in beta for macOS and Linux. ``` 1curl https://clickhouse.com/cli | sh ``` Copy command Tip: A `chctl` alias is created automatically for convenience.
