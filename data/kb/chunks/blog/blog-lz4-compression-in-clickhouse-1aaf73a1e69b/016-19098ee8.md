...
27 _mm_loadu_si128(reinterpret_cast<const __m128i *>(match)), 28 _mm_load_si128(reinterpret_cast<const __m128i *>(masks) + offset))); 29 30 match += masks[offset]; 31 32#else 33 copyOverlap16(op, match, offset); 34#endif 35} ``` Copy command Here \_mm\_shuffle\_epi8 is an [intrinsic](https://software.intel.com/sites/landingpage/IntrinsicsGuide/#text=_mm_shuffle_epi8), which compiles to the pshufb CPU instruction.
