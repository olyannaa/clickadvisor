...
when each might be appropriate. ### Delivery semantics \# Apache Kafka supports three message delivery semantics, listed here in ascending order of implementation complexity: - **At\-most\-once**: A message is delivered either one time only or not at all.
