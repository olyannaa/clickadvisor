...
for the database nodes while using instance types are suitable for an in\-memory cache for the ZooKeeper nodes. ## **Why does ClickHouse work so well for us?** \# ClickHouse did so well with helping us scale APM because:
