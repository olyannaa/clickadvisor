...
x, 5 toTypeName(x) AS type, 6 toJSONString(x) AS json 7 FORMAT Vertical" ``` Copy command ``` 1Row 1: 2────── 3x: ('Hello',123) 4type: Tuple( 5 a String, 6 b UInt8) 7json: {"a":"Hello","b":123} ``` Copy command --- Share this post
