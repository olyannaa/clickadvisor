...
4 server clickhouse_server 5 options ( 6 table '<clickhouse_table>' 7 ); ``` Copy command Before we do this for our HouseClick application, let's establish a few best practices when using the FDW with ClickHouse. ### Best practices \#
...
2	duration AS name,
3	count() AS value
4FROM default.uk_price_paid
5WHERE postcode1 = 'SL4'
6GROUP BY duration
7
8┌─name──────┬─value─┐
9│ leasehold │  6469 │
...
11└───────────┴───────┘
12
132 rows in set. Elapsed: 0.114 sec. Processed 28.11 million rows, 75.52 MB (247.22 million rows/s., 664.11 MB/s.)
```
Copy command
We use this specific query to drive a pie chart visual.

![pie_chart.png](/_next/image?url=%2Fuploads%2Fpie_chart_9c7b6ed5e0.png&w=2048&q=75)
