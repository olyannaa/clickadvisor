...
```
1SELECT station, min(measure), max(measure), round(avg(measure), 2)
2FROM s3('https://coiled-datasets-rp.s3.us-east-1.amazonaws.com/1trc/measurements-1*.parquet', 'AWS_ACCESS_KEY_ID', 'AWS_SECRET_ACCESS_KEY', headers('x-amz-request-payer' = 'requester'))
3GROUP BY station
4ORDER BY station ASC
5FORMAT `Null`
6
70 rows in set. Elapsed: 454.770 sec. Processed 111.11 billion rows, 279.18 GB (244.32 million rows/s., 613.88 MB/s.)
8Peak memory usage: 476.72 MiB.
```
Copy command
