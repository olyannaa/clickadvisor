...
MB/s.) Peak memory usage: 5.42 MiB. ``` Copy command ### Other supported functions \# Beyond `hasAnyTokens` and `hasAllTokens`, the text index also accelerates a number of other functions, though these predate the index and don't support preprocessor expressions:
