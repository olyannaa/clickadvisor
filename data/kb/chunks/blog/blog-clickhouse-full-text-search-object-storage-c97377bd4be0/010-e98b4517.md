...
TABLE docs ( `key` UInt64, `doc` String, INDEX idx(doc) TYPE text(tokenizer = splitByNonAlpha) ) ENGINE = MergeTree() ORDER BY key; ``` Copy command and the following query ``` SELECT count() FROM docs WHERE hasToken(doc, 'clickhouse'); ``` Copy command
...
![Rethinking full-text search for object storage #1514 Open (7).png](/_next/image?url=%2Fuploads%2FRethinking_full_text_search_for_object_storage_1514_Open_7_5a031aa537.png&w=2048&q=75)
...
![Rethinking full-text search for object storage #1514 Open (8).png](/_next/image?url=%2Fuploads%2FRethinking_full_text_search_for_object_storage_1514_Open_8_dc22ad458c.png&w=2048&q=75)
...
![Rethinking full-text search for object storage #1514 Open (9).png](/_next/image?url=%2Fuploads%2FRethinking_full_text_search_for_object_storage_1514_Open_9_82cf0a82e3.png&w=2048&q=75)
