...
speedup gained from adding the text index to a relatively small table of 10 million rows. ``` -- Create table CREATE TABLE job_listings ( `key` UInt64, `tags` Array(String) ) ENGINE = MergeTree ORDER BY key; ``` Copy command
