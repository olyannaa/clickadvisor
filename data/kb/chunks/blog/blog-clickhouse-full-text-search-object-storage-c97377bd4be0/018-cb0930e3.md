...
command ##### Other tokenizers \# The `array` tokenizer performs no tokenization, i.e. every row value is treated as a token (see function [array](https://clickhouse.com/docs/sql-reference/functions/array-functions#array)). ``` 'hello my name is John' -> ['hello my name is John'] ``` Copy command
