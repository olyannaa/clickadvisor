...
values, but doing this makes it easier to run jobs against multiple environments like dev and production. I suggest creating the following job parameters that can be modified at runtime: - CLICKHOUSE\_HOST - CLICKHOUSE\_PORT - CLICKHOUSE\_USERNAME - CLICKHOUSE\_PASSWORD

Note: parameters need to be supplied through the UI with a prefix of `--` for them to be picked up and used in scripts.

![8_awsglue.png](/_next/image?url=%2Fuploads%2F8_awsglue_cf1f66dee3.png&w=2048&q=75)
