...
TABLE trades_baselines ( 2 sym LowCardinality(String), 3 p50 Float32, 4 p95 Float32, 5 p99 Float32, 6 -- ... 7 computed_at DateTime 8) 9ENGINE = MergeTree 10ORDER BY sym; ``` Copy commandA **refreshable materialized view** repopulates it every hour:
...
12GROUP BY sym;
```
Copy commandRefreshable MVs atomically truncate and replace the target table on each
...
10LIMIT 50;
```
Copy commandRun queryThis query goes from \~1\.7s (recomputing baselines inline as a CTE) to
...
![Dashboard with bubble chart and detailed table](/_next/image?url=%2Fuploads%2Fhero_dashboard_5c8d3ce323.png&w=2048&q=75)
...
![Symbol drilldown showing candlesticks with volume bars and a table of anomalous trades](/_next/image?url=%2Fuploads%2Fsymbol_drilldown_2844f51814.png&w=2048&q=75)
