...
result.1 AS embedding 18FROM trades; ``` Copy commandEvery `INSERT INTO trades` fires this MV. The Python pool scores the batch and lands the result in `trades_embeddings`. There's no other mover, no other service, no separate scheduler. Just SQL.
...
10ORDER BY sym;
```
Copy commandA **refreshable materialized view** repopulates it every hour:
