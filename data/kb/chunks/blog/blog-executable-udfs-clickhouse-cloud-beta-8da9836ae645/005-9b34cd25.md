...
screen in the Cloud console. You give it a name, a zip containing your code and model files, and a few runtime parameters. ![ClickHouse Cloud UDF deployment page with argument list and runtime settings](/_next/image?url=%2Fuploads%2Fcloud_udf_deployment_95cd9b3a7d.png&w=2048&q=75) For `embed_trade` we use:
...
[`udf/cloud-deployment.md`](https://github.com/ClickHouse/stock-anomaly-udf/blob/main/udf/cloud-deployment.md) for the full
table.
...
```
Copy commandRun queryThe interesting part isn't *that* you can do this. It's *where* you can
put the call.
...
18FROM trades;
```
Copy commandEvery `INSERT INTO trades` fires this MV. The Python pool scores
