...
5.34 billion 6└────────────┘ 7 81 row in set. Elapsed: 0.004 sec. ``` Copy commandThe following shows the cardinality of each column, as well as the total cardinality if we were to represent this using a time series model.
...
37└───────────────────┘
```
Copy commandClearly, the number of time series here is significant, almost the same as the number of rows at 5\.2 billion.
...
```
Copy commandThis is an example of a query that would be challenging to execute in Prometheus. You would need to access many independent series in order to compute this.
