...
fully independent columns. - **Fast filtering with data pruning** \- A sparse index is built over the table’s sorting key, which in this case consists of commonly filtered dimensions materialized from the label map, followed by the timestamp.
