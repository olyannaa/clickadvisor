...
rows/s., 2.79 GB/s.) 19Peak memory usage: 726.64 MiB. ``` Copy commandThis is an example of a query that would be challenging to execute in Prometheus. You would need to access many independent series in order to compute this.
...
```
Copy commandIn the latter case, we read significantly less data thanks to the inverted index, as reflected in the query execution time. To narrow to a specific series, you would just add filters for all columns.
