...
structure of each row might not be the same as that of other rows or where we don’t want to break it out into individual columns. To use the JSON data type, we’ll need to enable the following:
...
```
1wget https://github.com/statsbomb/open-data/archive/refs/heads/master.zip
```
Copy command
...
```
1SELECT
2    replaceRegexpAll(splitByRegexp('/', _file)[-1], '.json', '') AS matchId,
3    json
4FROM file('master.zip :: **/data/events/*.json', JSONAsObject)
5LIMIT 1
6FORMAT Vertical
7
8Row 1:
