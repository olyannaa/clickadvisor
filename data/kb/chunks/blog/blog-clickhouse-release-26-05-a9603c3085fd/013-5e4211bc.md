...
subqueries but disable it in one of them: ``` 1SELECT * 2FROM (SELECT * FROM table1) t1 3NATURAL JOIN (SELECT * FROM table2 SETTINGS use_query_cache = 0) t2 4SETTINGS use_query_cache_for_subqueries = 1; ``` Copy command### Get started today
