...
4FORMAT `Null`
5SETTINGS join_algorithm = 'partial_merge';
6
70 rows in set. Elapsed: 33.796 sec. Processed 101.00 million rows, 3.67 GB (2.99 million rows/s., 108.47 MB/s.)
```
Copy command
...
```
1SELECT *
2FROM actors_unsorted AS a
3INNER JOIN roles AS r ON a.id = r.actor_id
4FORMAT `Null`
5SETTINGS join_algorithm = 'partial_merge';
6
7
80 rows in set. Elapsed: 44.872 sec. Processed 101.00 million rows, 3.67 GB (2.25 million rows/s., 81.70 MB/s.)
```
Copy command
