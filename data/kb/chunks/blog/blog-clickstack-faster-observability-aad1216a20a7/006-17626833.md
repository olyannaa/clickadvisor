...
`StatusCode` LowCardinality(String), 6 `count` SimpleAggregateFunction(sum, UInt64), 7 `avg__Duration` AggregateFunction(avg, UInt64) 8) 9ENGINE = AggregatingMergeTree 10ORDER BY (Timestamp, ServiceName, StatusCode); ``` Copy command And then define the incremental materialized view that continuously maintains those states as data is inserted:
...
```
138246 rows in set. Elapsed: 0.027 sec. Processed 41.22 thousand rows, 1.57 MB (1.52 million rows/s., 57.80 MB/s.) Peak memory usage: 21.34 MiB.
```
Copy command
...
![](/_next/image?url=%2Fuploads%2Fclickstack_mar2026_image6_a5473bd3d5.png&w=2048&q=75)
...
21. │ otel_v2  │ otel_traces_1m │     1 │ 41220 │     5 │
3   └──────────┴────────────────┴───────┴───────┴───────┘
41 row in set. Elapsed: 0.006 sec.
```
Copy command
