...
e.event_name -- ✗ blocked 13ORDER BY n DESC -- ✗ blocked 14LIMIT 100 -- ✗ blocked ``` Copy command A simpler subset of this query (drop the FILTER, percentile, and ROW\_NUMBER) would push down fully at this point:
...
### Step 4: \+ ordered\-set aggregates (`percentile_cont` → `quantile`) \#
