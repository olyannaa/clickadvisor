...
We used the same approach to get a data set for functional testing. After ClickHouse was released as open source in 2016, people began questioning these tests. ## Shortcomings of tests on private data \# Our performance tests:
...
```
1SELECT RegionID, uniq(UserID) AS visitors
2    FROM test.hits
3GROUP BY RegionID
4ORDER BY visitors DESC
5LIMIT 10
```
Copy command
