...
table. It is possible to restrict the export to a subset of columns using the SELECT statement: ``` 1UNLOAD ('SELECT * FROM some_table') 2TO 's3://my_bucket_name/some_table_' 3iam_role 'arn:aws:...' PARQUET 4ALLOWOVERWRITE ``` Copy command ![Exporting Data from Redshift to S3](/_next/image?url=%2Fuploads%2FExporting_Data_from_Redshift_to_S3_e75fe8d2a2.png&w=2048&q=75)
