...
aggregates, and sorts the data before applying a LIMIT. With the `graph` option, ClickHouse outputs the plan in [DOT](https://en.wikipedia.org/wiki/DOT_(graph_description_language)) format, which can be rendered to a PDF using [Graphviz](https://en.wikipedia.org/wiki/Graphviz). #### Example 1: One Parquet file, 4 cores \#
...
```
Copy command
![Blog-FormatsReads.007.png](/_next/image?url=%2Fuploads%2FBlog_Formats_Reads_007_e7a34f6398.png&w=2048&q=75)
