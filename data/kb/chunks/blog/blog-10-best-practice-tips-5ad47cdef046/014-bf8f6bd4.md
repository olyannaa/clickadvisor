...
parts DESC; ``` Copy command **Over\-partitioned tables:** ``` 1SELECT table, partition, count() AS parts 2FROM system.parts 3WHERE active 4GROUP BY table, partition 5HAVING parts > 10 6ORDER BY parts DESC; ``` Copy command ## 9\. Perfecting ReplacingMergeTree \#
