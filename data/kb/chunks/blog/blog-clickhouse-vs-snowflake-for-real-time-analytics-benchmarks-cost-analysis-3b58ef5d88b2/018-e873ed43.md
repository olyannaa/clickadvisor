...
the mean being 1\.7x faster. Results [here](https://github.com/ClickHouse/clickhouse_vs_snowflake/tree/main/downloads_per_day_by_system#results). As mentioned above, the subquery identifying the top systems for a project is an ideal candidate to optimize with a projection in ClickHouse and materialized view in Snowflake (with clustering). ![downloads_by_system_with_mv.png](/_next/image?url=%2Fuploads%2Fdownloads_by_system_with_mv_50405c0820.png&w=2048&q=75)
...
3    count() AS c
4FROM pypi
5WHERE (project = 'boto3') AND (date >= (CAST('2023-06-23', 'Date') - toIntervalDay(90)))
6GROUP BY file.type
7ORDER BY c DESC
8LIMIT 10
```
...
![top_file_type_per_project.png](/_next/image?url=%2Fuploads%2Ftop_file_type_per_project_6bfbc55e4e.png&w=2048&q=75)
