...
the `(to_date(timestamp), project)` clustering key. **Based on the configurations and schemas optimized for query speed, our results show that ClickHouse Cloud outperforms Snowflake in compression by 38% (0\.902TiB vs. 1\.33TiB).** #### Clustering time \& costs in Snowflake \#
