...
```
1SELECT splitByChar('-', project)[1] as base 
2from pypi 
3GROUP BY base 
...
3    count() AS c
4FROM pypi
5WHERE (project LIKE '%google%') AND (date >= (CAST('2023-06-23', 'Date') - toIntervalDay(90)))
6GROUP BY project
7ORDER BY c DESC
8LIMIT 10
```
...
![top_sub_projects.png](/_next/image?url=%2Fuploads%2Ftop_sub_projects_2f9715f79e.png&w=2048&q=75)
