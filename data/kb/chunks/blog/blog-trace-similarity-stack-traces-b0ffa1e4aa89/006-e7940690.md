...
identical traces and distinguish the significant ones among them \# Once the stack traces are cleaned, we can start analyzing them. The already processed traces are in the `crash_issues` table; we use these to find the existing issues.
