...
10FROM noaa_codec_v3 11WHERE (date > '1970-01-01') AND (station_id IN ( 12 SELECT station_id 13 FROM blogs.stations 14 WHERE country_code = 'US' 15)) 16GROUP BY floor(elevation, -2) AS elevation_range 17ORDER BY elevation_range ASC 18Format Null ``` Copy commandRun query

```
10 rows in set. Elapsed: 1.132 sec. Processed 330.29 million rows, 6.39 GB (291.78 million rows/s., 5.64 GB/s.)
```
Copy command
...
![encode_compress.png](/_next/image?url=%2Fuploads%2Fencode_compress_ac197d09d2.png&w=2048&q=75)
...
![delta_encoding.png](/_next/image?url=%2Fuploads%2Fdelta_encoding_074dd1dbc7.png&w=2048&q=75)
