...
key, 'value', SpanAttributes[key]), mapKeys(SpanAttributes)) AS tags, 22 arrayMap(key -> map('key', key, 'value', ResourceAttributes[key]), mapKeys(ResourceAttributes)) AS serviceTags 23FROM otel_traces 24WHERE (TraceId = trace_id) AND (Timestamp >= start) AND (Timestamp <= end) 25ORDER BY startTime ASC ``` Copy command [![](/_next/image?url=%2Fuploads%2Fgrafana_trace_34d527cf86.png&w=2048&q=75)](/uploads/grafana_trace_34d527cf86.png)
...
[![](/_next/image?url=%2Fuploads%2Fgrafana_trace_66c848f8e5.gif&w=2048&q=75)](/uploads/grafana_trace_66c848f8e5.gif)
