...
please upgrade \- it is a Long Term Support release. ## Query of the Month: Deleting data can make you feel better \# For this month, let’s keep it simple. And tongue in cheek, just a little bit.
...
```
1CREATE TABLE mymoney engine = MergeTree ORDER BY coin AS
2SELECT 'USDC' AS coin, 1000 AS amount, 1.0002 AS cost, 0.9999 AS price
3UNION ALL SELECT 'USDT', 1000, 1.0001, 1.00
4UNION ALL SELECT 'USTC', 1000, 1.00, 0.0275
```
Copy command
And let’s calculate our portfolio return:

```
1SELECT sum(amount * cost) cost, sum(amount * price) value,
2value - cost gain_loss, 1 - value / cost pct  FROM mymoney
```
Copy command
...
```
1SET allow_experimental_lightweight_delete = 1
2DELETE FROM mymoney WHERE coin = 'USTC'
```
Copy command
