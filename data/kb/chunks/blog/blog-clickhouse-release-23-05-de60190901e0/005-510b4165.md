...
2
3SELECT
4	toYear(toDate(date)) AS year,
5	round(avg(price)) AS price,
6	bar(price, 0, 1000000, 80)
7FROM file('house_prices.parquet')
8GROUP BY year
9ORDER BY year ASC
10
1129 rows in set. Elapsed: 0.367 sec.
12
13--23.5
14
15SELECT
...
19FROM file('house_prices.parquet')
20GROUP BY year
21ORDER BY year ASC
22
2329 rows in set. Elapsed: 0.240 sec.
```
Copy command
