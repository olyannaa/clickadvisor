...
19 .WithExternalHttpEndpoints() 20 .WithEnvironment("Analytics__BaseUrl", analytics.GetEndpoint("http")) 21 .WaitFor(analytics); 22 23builder.AddProject("traffic") 24 .WithEnvironment("Gateway__BaseUrl", gateway.GetEndpoint("http")) 25 .WaitFor(gateway); 26 27builder.Build().Run(); ``` Copy command It's worth taking a deeper look into some of the calls to understand what is happening under the hood:
...
## Using the Driver Client in Services \#
