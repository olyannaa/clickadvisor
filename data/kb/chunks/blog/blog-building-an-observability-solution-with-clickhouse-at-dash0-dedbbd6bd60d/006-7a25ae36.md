...
`StartTimeUnix` DateTime64(9) CODEC(Delta, LZ4), `TimeUnix` DateTime64(9) CODEC(Delta, LZ4), `Value` Float64 CODEC(LZ4), ) ENGINE MergeTree PARTITION BY toDate(TimeUnix) ORDER BY (MetricHash, TimeUnix) ``` Copy command The `MetricHash` acts as the join column with the metadata table, further described below.
