...
Null\-hypothesis \# Hypothesis testing is a fundamental concept in statistics and plays a crucial role in drawing inferences and making decisions. The null hypothesis is used to assess the validity of claims or research hypotheses, using sample data.
...
```
1SELECT explicit, varPop(danceability), count() AS c
2FROM spotify
3GROUP BY explicit
4
5┌─explicit─┬─varPop(danceability)─┬──────c─┐
6│ false	│ 0.029643285945200746    │ 104253 │
7│ true 	│ 0.029892924927367216    │   9747 │
8└──────────┴──────────────────────┴────────┘
9
102 rows in set. Elapsed: 0.840 sec. Processed 77.00 thousand rows, 8.81 MB (91.69 thousand rows/s., 10.49 MB/s.)
```
Copy command
With these properties satisfied, performing a t\-test in ClickHouse is simple:

```
1SELECT studentTTest(danceability, explicit)
2FROM spotify
3
4┌─studentTTest(danceability, explicit)─┐
5│ (-41.67680374902913,0)           	   │
6└──────────────────────────────────────┘
7
81 row in set. Elapsed: 0.841 sec. Processed 2.00 thousand rows, 232.29 KB (2.38 thousand rows/s., 276.13 KB/s.)
```
Copy command
