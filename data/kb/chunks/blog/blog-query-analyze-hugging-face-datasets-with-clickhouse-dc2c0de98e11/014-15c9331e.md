...
Copy command The key value here is the 2nd value in our tuple, the p\-value. This is the probability of obtaining results as extreme as, or more extreme than, the observed results, assuming the null hypothesis is true.
...
6GROUP BY explicit
7
8┌─explicit─┬──────varPop(valence)─┬──────c─┐
9│ false	   │  0.06861382619038442 │ 104253 │
10│ true 	   │ 0.052252604489216155 │   9747 │
11└──────────┴──────────────────────┴────────┘
12
132 rows in set. Elapsed: 0.857 sec. Processed 2.00 thousand rows, 232.29 KB (2.33 thousand rows/s., 270.96 KB/s.)
```
Copy command
Welch's t\-test is equally as simple in ClickHouse:

```
1SELECT welchTTest(valence, if(explicit, 1, 0))
2FROM spotify
3
4┌─welchTTest(valence, if(explicit, 1, 0))──┐
5│ (1.2775135699871494,0.20144516672703286) │
6└──────────────────────────────────────────┘
7
81 row in set. Elapsed: 0.839 sec. Processed 40.00 thousand rows, 4.53 MB (47.65 thousand rows/s., 5.40 MB/s.)
```
Copy command
