...
Elapsed: 0.124 sec. Processed 617.97 million rows, 617.97 MB (5.00 billion rows/s., 5.00 GB/s.) 12Peak memory usage: 139.03 MiB. ``` Copy command Within these commit events, we can inspect the event types using the [JSON type path syntax](https://clickhouse.com/docs/en/sql-reference/data-types/newjson#reading-json-paths-as-subcolumns):
