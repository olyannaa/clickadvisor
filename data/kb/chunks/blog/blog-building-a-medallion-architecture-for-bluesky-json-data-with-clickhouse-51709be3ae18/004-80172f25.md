...
million rows/s., 2.83 GB/s.) 9Peak memory usage: 4.85 GiB. ``` Copy command We can sample the data using the same function, casting each row to the JSON type and using the `PrettyJSONEachRow` format to obtain a readable result.
