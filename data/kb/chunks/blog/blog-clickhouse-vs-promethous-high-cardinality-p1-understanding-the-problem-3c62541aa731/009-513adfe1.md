...
12}[5m])
```
Copy commandThis calculates the average response time over the last five minutes for the specific `host`, `application`, `request path`, and `status` by dividing the rate of total response time by the rate of request count.
