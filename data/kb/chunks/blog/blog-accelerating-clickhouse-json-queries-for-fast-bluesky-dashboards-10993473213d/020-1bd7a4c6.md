...
Copy command This is the incremental materialized view: ``` 1CREATE MATERIALIZED VIEW bluesky.handle_per_user_mv 2TO bluesky.handle_per_user 3AS 4SELECT 5 data.identity.did::String AS did, 6 argMax(data.identity.handle, bluesky_ts) AS handle 7FROM bluesky.bluesky 8WHERE (kind = 'identity') 9GROUP BY did; ``` Copy command
