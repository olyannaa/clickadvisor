...
ingest new [events](https://github.com/bluesky-social/jetstream?tab=readme-ov-file#example-events) (e.g. post, like, repost). ### How we store Bluesky JSON data in ClickHouse \# Below is the schema of the ClickHouse table storing the full, constantly growing Bluesky dataset, accessible through our [ClickHouse SQL playground](/blog/accelerating-clickhouse-json-queries-for-fast-bluesky-dashboards#test-all-queries-live):
