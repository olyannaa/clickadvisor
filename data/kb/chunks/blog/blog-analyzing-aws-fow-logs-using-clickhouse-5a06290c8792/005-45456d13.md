...
```
1aws s3 sync s3://<prefix>-us-east-2-flow-logs.clickhouse.cloud .
2
3for f in **/*.log.parquet
4do
5  echo "Importing $f"
6  cat $f | clickhouse client --query "INSERT INTO flowlogs_us_east_2_v4 FORMAT Parquet"  --host <instance>.us-west-2.aws.clickhouse.cloud  --secure  --port 9440  --password <password>
7done
```
Copy command
