...
smaller improvements that make ClickStack easier to use day to day. These changes focus on usability, smoother workflows, and simpler deployment, helping teams get value faster while steadily improving the overall experience. ### Filtering tables with HAVING \#
...
```
Copy commandThe latest version of ClickStack now supports `HAVING` when querying tables in the UI. This allows users to filter results using any aggregate expression, not just raw columns. In practice, this means you can filter by an alias such as `avg_duration > 100`, or define an entirely new aggregate in the filter itself, for example `median(Duration) > 100`, giving you far more flexibility when exploring and refining aggregated results.

![having_table_clickstack.png](/_next/image?url=%2Fuploads%2Fhaving_table_clickstack_21e0b63ee7.png&w=2048&q=75)
