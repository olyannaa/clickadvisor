...
KeyValuePair<string, object?>("status", result.Status), 28 new KeyValuePair<string, object?>("payment_method", request.PaymentMethod)); 29 30 return result; 31} ``` Copy command The fraud check span includes an event when the score is suspicious. All of these will show up in ClickStack's trace waterfall:
...
## Database layer with SQLite and Entity Framework Core \#

Both services persist data to SQLite using Entity Framework Core.

### Auto\-instrumented database spans \#
