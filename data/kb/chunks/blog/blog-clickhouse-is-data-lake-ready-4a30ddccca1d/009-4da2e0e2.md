...
then, we can write some queries against it: ``` ``` 1SELECT 2 toHour(pickup_datetime) AS hour, 3 avg(trip_distance) AS avg_distance, 4 avg(total_amount) AS avg_fare, 5 count() AS trips 6FROM nyc_taxi 7GROUP BY hour 8ORDER BY hour ASC; ``` ```
