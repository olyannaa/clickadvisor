...
before any data is read**, further accelerating Top\-N queries as data sizes grow. ## How ClickHouse already optimizes Top\-N queries \# ClickHouse already applies several optimizations to run Top\-N queries efficiently and fast: 1. [Streaming](https://clickhouse.com/docs/sql-reference/statements/select#implementation-details) Top\-N (bounded memory):
