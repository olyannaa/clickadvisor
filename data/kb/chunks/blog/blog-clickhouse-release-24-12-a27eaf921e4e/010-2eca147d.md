...
``` 1JOIN ...ON a=b AND (x OR y OR z) ``` Copy command This behavior can be enabled by setting the new `optimize_extract_common_expressions` setting to `1`. Because this setting is currently experimental, the default value is currently `0`.
