...
gives you more control over the process if you need it. - **Default table engine** – You can now set a default table engine so you no longer have to specify one when creating a table. For example:

```
1SET default_table_engine = 'MergeTree'
2CREATE TABLE table1 (x int PRIMAY KEY (x))
```
Copy command
...
```
1SET allow_experimental_nlp_functions = 1
2SELECT detectLanguage('Ich bin ein Berliner')
3// returns 'de'
```
Copy command
...
```
1dictGetOrNull(dict, attr, 1)
2dictGetOrNull(dict, attr, '1')
3dictGetOrNull(dict, attr, 1.0)
4dictGetOrNull(dict, attr, toUInt8(1))
5dictGetOrNull(dict, attr, toInt256(1))
```
Copy command
