...
(); ``` Copy command And at query time, we specify how many (most significant) bits to take: ``` 1SELECT id, name FROM vectors 2ORDER BY L2DistanceTransposed(vector, target, 10) 3LIMIT 10; ``` Copy command ## Query condition cache \#
