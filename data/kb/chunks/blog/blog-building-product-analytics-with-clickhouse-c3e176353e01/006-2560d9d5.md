...
`component` String, 12 `event` String, 13 `interaction` LowCardinality(String), 14 `payload` String, 15 `message` String 16) 17ENGINE = MergeTree 18ORDER BY created_at ``` Copy command Elements of this schema are specific to Galaxy, while others are reusable. More specifically:
