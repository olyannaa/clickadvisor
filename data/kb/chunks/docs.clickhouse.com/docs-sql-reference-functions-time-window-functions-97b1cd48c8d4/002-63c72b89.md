...
hopping window. Since one record can be assigned to multiple hop windows, the function only returns the bound of the first window when hop function is used without `WINDOW VIEW`. **Syntax** ``` hopStart(time_attr, hop_interval, window_interval[, timezone]) ``` **Arguments**
