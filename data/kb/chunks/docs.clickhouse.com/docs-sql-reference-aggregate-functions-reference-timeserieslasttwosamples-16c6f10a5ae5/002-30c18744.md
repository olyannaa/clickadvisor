...
to 2\. The first array contains the timestamps of sampled time series, the second array contains the corresponding values of the time series. [`Tuple(Array(DateTime), Array(Float64))`](/docs/sql-reference/data-types/tuple) **Examples** **Example table for raw data, and a table for storing re\-sampled data**
