...
an exception in case of an error. Supported arguments: - Values of type (U)Int\*. - String representations of (U)Int8/16/32/128/256\. - Values of type Float\*, including `NaN` and `Inf`. - String representations of Float\*, including `NaN` and `Inf` (case\-insensitive).
